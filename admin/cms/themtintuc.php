...
if($_SERVER["REQUEST_METHOD"]=="POST")
            {
                ...
                if (empty($error))
                {                      
                    $query = "insert into tintucsach(tieude,anhnen,noidung,ngaytao) values('$tieude', '$anhnen', '$noidung',CURDATE())";
                    $count=$db->exec($query);
                    if($count>0)
                    {
                        move_uploaded_file($_FILES['fImg']['tmp_name'], "../images/source/$anhnen");
